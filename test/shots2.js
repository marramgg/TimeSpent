// Later-game states: PT language, a day-4 wish, the toy stall, settings. Usage: node test/shots2.js [outdir]
...
(async () => {
  ...
  // jump to day 4, when the toy stall and wishes open, with a wish and some coins
  await page.evaluate(() => { const s = TS.state; s.day = 4; s.coins = 7; s.wish = 'bike'; s.toys = ['ball']; s.msgs = [{ key: 'morning', day: 4 }, { key: 'newToys' }]; TS.settings.voice = false; });
  await page.evaluate(() => TS.act('rest')); await idle(); await page.waitForTimeout(300);
  await page.screenshot({ path: `${out}/pt-2-day4.png` });
  await tap('[data-place="shops"]');
  await page.screenshot({ path: `${out}/pt-3-travel.png` });   // day 4: walk or bus, so the sheet asks
  await tap('[data-mode="walk"]');
  await page.screenshot({ path: `${out}/pt-4-shops.png` });
